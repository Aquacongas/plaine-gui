...
fn main() {
    ...
    if origin == "none" {
        ...
        if std::env::var("PLAINE_REQUIRE_BUILD_ID").as_deref() == Ok("1") {
            panic!(
                "PLAINE_REQUIRE_BUILD_ID=1 and this build cannot name its commit. \
                 A release binary that cannot be tied back to a commit must not be \
                 produced at all: it would carry our signature and no traceability."
            );
        }
    }
...
fn identify(manifest: &Path, workspace: &Path) -> (String, String, String) {
    if let Some(v) = std::env::var_os("PLAINE_GIT_SHA") {
        ...
        panic!(
            "PLAINE_GIT_SHA is set to {:?}, which is not 40 hex characters. \
             Refusing rather than falling back: a build system that sets this \
             wrong must not be told the build was fine.",
            v
        );
    }
