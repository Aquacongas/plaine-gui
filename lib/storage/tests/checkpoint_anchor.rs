...
fn anchor_row_survives_reopen() {
    ...
    let want = record(4_242, 0xAB);
    {
        ...
        assert_eq!(r.checkpoint_anchor().expect("read"), None, "a fresh store holds no anchor");
...
fn bad_size_record_refused() {
    ...
    assert!(c.put_checkpoint_anchor(&[]).is_err(), "an empty row is not a record");
    assert!(
        c.put_checkpoint_anchor(&vec![0u8; ANCHOR_RECORD_CAP_BYTES + 1]).is_err(),
        "the cap is this crate's own bound and must hold whatever the writer says"
    );
    assert_eq!(r.checkpoint_anchor().expect("read"), None, "a refused write left a row");

    c.put_checkpoint_anchor(&vec![7u8; ANCHOR_RECORD_CAP_BYTES]).expect("at the cap");
    assert_eq!(
