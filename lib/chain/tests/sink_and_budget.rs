...
fn one_call_per_source_per_epoch() {
    ...
    assert_eq!(
        calls, 1,
        "one failed call per source per tip epoch, and no more"
    );
    ...
    assert!(
        micros_per_byte_now < 0.05,
        "got {micros_per_byte_now} us/byte"
    );
    assert!(
...
fn interpreter_budget_per_source_and_global() {
    ...
    for s in 0..SOURCES {
        for _ in 0..120 {
            let _ =
                r.cm.submit_headers(1_000 + s as u32, std::slice::from_ref(&raws[i]));
            i += 1;
        }
    }
    ...
    assert_eq!(
        calls, SOURCES,
        "one failed call per source, not one burst per source"
    );
    ...
    assert!(
        aggregate <= class,
        "the two tiers may never sum above the cap"
    );
    assert!(
    ...
    assert!(
        128 * 100_000 / 10 > class,
        "tier 1 alone still exceeds the class cap"
    );
}
...
fn oversized_batch_refused() {
    ...
    assert_eq!(
        r.cm.index().len(),
        arena_before,
        "nothing was staged or connected"
    );

    assert_eq!(
        r.cm.source_count(),
        sources_before,
        "no per-source state was allocated"
    );

    assert!(r
        .cm
        .submit_headers(11, &orphans[..MAX_HEADERS_PER_MSG])
        .is_ok());
}
...
fn cheap_reject_skips_sig() {
    ...
    assert!(matches!(
        r.cm.submit_tx(TxOrigin::Peer(7), stale)
            .expect_err("refused"),
...
fn mempool_ingress_per_source_and_global() {
    ...
    assert_eq!(
        past_bucket, 100,
        "the per-source rate is the declared 100 tx/s"
    );
...
fn refusing_sink_keeps_old_chain() {
    ...
    r.store
        .fail_next_commit(SinkError::Invalid("refused on purpose"));
    ...
    assert!(
        r.cm.halted().is_none(),
        "Invalid is refusal, not corruption"
    );
...
fn fatal_sink_halts_before_republish() {
    ...
    r.offer(4, &blocks_above(&attacker, 20));
    r.store
        .fail_next_commit(SinkError::Fatal("writer lost track"));

    let e = r.cm.advance().expect_err("fatal");
    assert!(matches!(e, Reject::Halted { .. }), "got {e:?}");
    assert!(
        r.cm.halted().is_some(),
        "halting is a state, not a log line"
    );
    assert_eq!(
        r.cm.tip().hash,
        before_tip,
...
fn rollback_double_write_restores() {
    ...
    assert!(
        m_before.balance > 0,
        "the miner must already hold coinbase value"
    );
    ...
    let expected: u128 = m_before.balance
        + (21..=23)
            .map(plaine_consensus::emission::block_reward)
            .sum::<u128>();
    assert_eq!(
        r.store.account(&m_addr).balance,
        expected,
...
fn mid_branch_failure_restores_state() {
    ...
    assert!(
        r.cm.last_branch_failure().is_some(),
        "the cause survives for the operator"
    );
}
