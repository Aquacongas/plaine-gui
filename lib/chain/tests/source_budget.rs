...
fn older_ancestor_children_skip_interpreter() {
    ...
    assert_eq!(
        r.pow.calls(),
        0,
        "not one interpreter call for 180 forged headers"
    );
    assert_eq!(
        r.cm.index().len(),
        arena_before,
        "nothing entered the arena"
    );
    assert_eq!(
        last_staged, 180,
        "what it bought instead: 180 parked staging slots"
    );
...
fn peer_ceiling_one_failed_call_per_epoch() {
    ...
    assert_eq!(
        r.pow.calls(),
        128,
        "one failed call per source, and the second is refused"
    );
    ...
    assert!(
        per_second * 50 < p.class_rate_micros_per_sec(),
        "{per_second} us/s"
    );
}
...
fn parked_header_does_not_lock_table() {
    ...
    let a =
        r.cm.submit_headers(9_999, &[blk.rec.raw])
...
fn honest_peer_keeps_reserve() {
    ...
    assert_eq!(
        r.pow.calls(),
        2 + 83,
        "the flood is bounded by the shared half plus its own reserve"
    );
    ...
    assert_eq!(
        a.connected, 1,
        "the honest peer was starved out of the class budget"
    );
    assert_eq!(r.pow.calls(), 86);

    let burst_ceiling =
        p.class_shared_burst_micros() + p.class_reserve_burst_micros(3_000) * p.max_sources as u64;
...
fn class_aggregate_counts_rows() {
    ...
    assert_eq!(
        ...
        500_000,
        "250 x P ms/s at the VPS floor P=2"
    );
    assert!(
        p.max_sources <= p.max_peers,
        "one reserve per row, so rows may not exceed peers"
    );
    assert!(
    ...
    assert_eq!(
        old.class_aggregate_rate_micros_per_sec(),
        250_000 + 256 * 1_953
    );
...
fn fatal_sink_deep_replay_exact() {
    ...
    r.store
        .fail_next_commit(SinkError::Fatal("the deep writer lost track"));
    ...
    assert_eq!(
        r.cm.tip().hash,
        before_tip,
        "the arena still names the old tip"
    );
    assert_eq!(
    ...
    assert_eq!(
        r.store.canonical_hashes(),
        before_canon,
        "canonical index unmoved"
    );
    assert_eq!(
    ...
    assert!(
        r.observed(|c| matches!(c, Condition::DeepReplay { .. })),
        "the deep path did run"
    );
...
fn fatal_sink_fast_path_exact() {
    ...
    r.store
        .fail_next_commit(SinkError::Fatal("the writer lost track"));
    ...
    assert!(matches!(
        r.cm.submit_block(&blk.rec.hash, blk.body.clone()),
        Err(Reject::Halted { .. })
    ));
}
...
fn invalidating_tip_no_fork_choice() {
    ...
    assert_eq!(
        r.tip_hash(),
        tip_before,
        "still on the chain we marked invalid"
    );
...
fn dead_branch_not_reverified() {
    ...
    let _ =
        r.cm.submit_headers_solicited_steady(9, &raws)
            .expect("not halted");
    assert_eq!(
        r.pow.calls(),
        1,
        "strictly ascending, abort at the first failure"
    );

    for _ in 0..3 {
        let _ =
            r.cm.submit_headers_solicited_steady(9, &[])
                .expect("not halted");
    }
    assert_eq!(
        r.pow.calls(),
        1,
        "saying nothing costs nothing once the branch is dead"
    );
}
...
fn quota_epoch_rolls_on_a_stalled_tip() {
    ...
    assert_eq!(
        r.pow.calls(),
        2,
        "a stalled tip must not lock a peer out forever"
    );
}
...
fn refused_replacement_keeps_incumbent() {
    ...
    assert!(
        bulky.len() + bytes_before > 400,
        "the fixture must force the eviction loop"
    );
    let verdict = pool.submit(
        bulky,
        rich,
        u128::MAX / 4,
        0,
    ...
    assert!(
        pool.get(&incumbent_id).is_some(),
        "a refused replacement ATE the incumbent"
    );
    assert!(
        pool.get(&incumbent_id).expect("pooled").executable,
        "it is executable again"
    );
    assert_eq!(pool.len(), 2);
    assert_eq!(
        pool.bytes(),
        bytes_before,
        "the byte accounting came back with it"
    );
}
